...
@dataclass
class MAGEConfig(LoraConfig):
    """
    This is the configuration class to store the configuration of a [`~peft.MAGE`]
    """
    task_embedding_dim: int = field(default=64)
    expert_num: int = field(default=4)

    def __post_init__(self):
        self.peft_type = PeftType.MAGE


class MAGEModel(LoraModel):
    """
    Create MAGE (MAGE based LoRA) model from a pretrained transformers model.
    """
    ...
    def add_adapter(self, adapter_name, config=None):
        if config is not None:  # get the lora config
            ...
            config = self._prepare_clitmage_config(config, model_config)   # load config
            self.peft_config[adapter_name] = config # subsititue the original config
        self._find_and_replace(adapter_name)
        if len(self.peft_config) > 1 and self.peft_config[adapter_name].bias != "none":
            raise ValueError(
                "MAGEModel supports only 1 adapter with bias. When using multiple adapters, set bias to 'none' for all adapters."
            )
    ...
    def _create_new_module(self, lora_config, adapter_name, target):
        ...
        if loaded_in_8bit and isinstance(target, bnb.nn.Linear8bitLt):
            ...
            new_module = MAGELinear(adapter_name, in_features, out_features, bias=bias, **kwargs)
    ...
    @staticmethod
    def _prepare_clitmage_config(peft_config, model_config):
...
class MAGELayer(LoraLayer):
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        if r > 0:
            self.lora_A.update(nn.ModuleDict({adapter_name: MAGELinearA(self.in_features, r, self.expert_num)}))
            self.lora_B.update(nn.ModuleDict({adapter_name: MAGELinearB(r, self.out_features, self.expert_num)}))
...
class MAGELinear(nn.Linear, MAGELayer):
    # Lora implemented in a dense layer
    # nn.Linear is the pretrained weights in LLM, MAGELayer is the designed trainable Lora 
    def __init__(
        self,
        adapter_name: str,
        in_features: int,
        ...
        **kwargs,
        ...
        MAGELayer.__init__(self, in_features=in_features, out_features=out_features, expert_num=self.expert_num)
...
class MAGELinearA(nn.Module):
    '''MAGE based LoRA block'''
    def __init__(self, in_features, out_features, expert_num) -> None:
        ...
        for _ in range(self.expert_num):
            self.loraA.append(MAGEExpert(self.in_features, self.r))
...
class MAGELinearB(nn.Module):
    ...
    def __init__(self, in_features, out_features, expert_num) -> None:
        ...
        for _ in range(self.expert_num):
            self.loraB.append(MAGEExpert(self.r, self.out_features))
...
class MAGERouter(nn.Module):
    ...
    def __init__(self, config: MAGEConfig):
        super().__init__()
