...
# image comprehension
image_path = 'demo_images/solvay.jpg'
...
question = 'What historical event is shown in the picture?'
...
image_path = 'demo_images/shanghai.png'
image = Image.open(image_path).convert('RGB')
image_tensor, patch_pos_tensor = process_anyres_image(image, image_transform, grid_pinpoints, base_resolution)
embeds_cmp_mask = torch.tensor([True]*image_tensor.shape[0]).to(device, dtype=torch.bool)
...
# detection
image_path = 'demo_images/office.png'
image = Image.open(image_path).convert('RGB')
image_tensor, patch_pos_tensor = process_anyres_image(image, image_transform, grid_pinpoints, base_resolution)
embeds_cmp_mask = torch.tensor([True]*image_tensor.shape[0]).to(device, dtype=torch.bool)
...
question = 'Is there anything in the image that I can use to search for information? Show me the location.'
prompt = instruction_prompt.format_map({'instruction': image_tokens + question})
