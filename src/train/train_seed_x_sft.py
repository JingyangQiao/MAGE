...
def ema_replace(ema_modules, agent_model):
    for name, param in agent_model.named_parameters():
        if param.requires_grad and name in ema_modules:
            print("Before param", param.data)
            nan_exist = torch.isnan(ema_modules[name].data).any()
            if not nan_exist:
                param.data = ema_modules[name].data
            print("After param", param.data)
    return agent_model
...
def train():
    ...
    task_name = train_dataset_cfg["datapipes"][0]["dataset_name"]
    for n, p in agent_model.named_parameters():
        # For Training ImageNet/OCRVQA/Grounding
        if "VQAv2" in task_name or "ImageNet" in task_name or "OCRVQA" in task_name or "Grounding" in task_name:
            if "loraA.3." in n or "loraB.3." in n:
                p.requires_grad = False
                text_input, image_input, text_output, image_output = True, True, True, False
        # For Training Flickr30K
        if "Flickr30k" in task_name:
            if "loraA.1." in n or "loraB.1." in n or "loraA.2." in n or "loraB.2." in n:
                p.requires_grad = False
                text_input, image_input, text_output, image_output = True, False, False, True
        # For Training HQEdit
        if "HQEdit" in task_name:
            if "loraA.2." in n or "loraB.2." in n:
                p.requires_grad = False
                text_input, image_input, text_output, image_output = True, True, False, True
    ...
    for epoch in range(args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(agent_model):
                ...
                for name, param in agent_model.named_parameters():
                    if param.requires_grad:
                        if name not in ema_modules:
                            ema_modules[name] = copy.deepcopy(param)
                        else:
                            with torch.no_grad():
                                if ("loraA.0." in name or "loraB.0." in name) and text_input:
                                    try:
                                        part_a = param.data - ema_modules[name].data  # theta_t - theta_{t-1}^*
                                        part_d = param.grad.detach() ** 2  # Hessian
                                        part_c = torch.matmul(part_a.T, part_d)
                                        part_c_reg = part_c + 1e-5 * torch.eye(part_c.size(-1), device=part_c.device)
                                        part_c = torch.linalg.inv(part_c_reg.float()).to(torch.bfloat16)
                                        part_b = param.grad.detach() + 1  # L' + I
                                        alpha = torch.matmul(part_b, part_c)
                                        ema_modules[name].data = ema_modules[name].data * alpha + param.data * (1 - alpha)
                                    except:
                                        ema_modules[name].data = ema_modules[name].data * 0.99 + param.data * 0.01
                                if ("loraA.1." in name or "loraB.1." in name) and image_input:
                                    try:
                                        part_a = param.data - ema_modules[name].data  # theta_t - theta_{t-1}^*
                                        part_d = param.grad.detach() ** 2  # Hessian
                                        part_c = torch.matmul(part_a.T, part_d)
                                        ...
                                        ema_modules[name].data = ema_modules[name].data * 0.99 + param.data * 0.01
                                if ("loraA.2." in name or "loraB.2." in name) and text_output:
                                    try:
                                        part_a = param.data - ema_modules[name].data  # theta_t - theta_{t-1}^*
                                        part_d = param.grad.detach() ** 2  # Hessian
                                        part_c = torch.matmul(part_a.T, part_d)
                                        ...
                                        ema_modules[name].data = ema_modules[name].data * 0.99 + param.data * 0.01
                                if ("loraA.3." in name or "loraB.3." in name) and image_output:
                                    try:
                                        part_a = param.data - ema_modules[name].data  # theta_t - theta_{t-1}^*
                                        part_d = param.grad.detach() ** 2  # Hessian
                                        part_c = torch.matmul(part_a.T, part_d)
            ...
            if accelerator.sync_gradients:
                ...
                if global_step % args.save_steps == 0:
                    agent_model = ema_replace(ema_modules, agent_model)
